...
public class RecordPage {
   ...
   public int currentId() {
    ...
    /**
     * Return the number of slots belongs to record page using computation below.
     * We have a block and we are dividing this block according to slot size.
     * @return number of slots in the block.
     */
   public int slots() {
       return BLOCK_SIZE / slotsize;
   }

    /**
     * Takes slot number as a parameter and moves next INUSE slot
     * before the specified slot number.
     * @param slot specified slot number
     * @return found slot number otherwise return -1
     */
   public int nextBefore(int slot) {
       currentslot = slot - 1;
       while (isValidSlot() && currentslot > -1){
           int position = currentpos();
           if (tx.getInt(blk, position) == INUSE)
               return currentslot;
           currentslot--;
       }
       return -1;
