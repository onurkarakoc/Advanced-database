...
public class RecoveryMgr {
   ...
   /**
    * This function takes one parameter which is list of
    * active transaction ids and writes a non-quiescent checkpoint
    * record to the log.
    * @param activeTransactions
    */
   public void checkpoint(ArrayList<Integer> activeTransactions) {
      int lastSeqNum = new NQCheckpoint(activeTransactions).writeToLog();
      SimpleDB.logMgr().flush(lastSeqNum);
      System.out.print("NQ CHECKPOINT: Transactions ");
      for (int transaction: activeTransactions)
    ...
    * Added recovering operation for non-quiescent checpoint.
    */
   private void doRecover() {
      Collection<Integer> finishedTxs = new ArrayList<Integer>();
      // We'll keep active transactions in this list.
      ArrayList<Integer> nQCheckpointsTransactions = new ArrayList<>();
      Iterator<LogRecord> iter = new LogRecordIterator();
      // We'll use this flag to determine NQCKPT is worked at least once or not. This is a just signature for us.
      boolean flag = false;
      while (iter.hasNext()) {
         LogRecord rec = iter.next();
         ...
         if (rec.op() == NQCKPT){
            // Sign the signature.
            flag = true;
            NQCheckpoint transactions = (NQCheckpoint) rec;
            // If a transaction has not finished yet means that it is still active.
            for (Integer transaction : transactions.txNumbers()) {
               if (!finishedTxs.contains(transaction)) {
                  nQCheckpointsTransactions.add(transaction);
               }
            }
         }
         // All work is done finish the recovering operation.
         if (nQCheckpointsTransactions.size() == 0 && flag)
            return;
         if (rec.op() == COMMIT || rec.op() == ROLLBACK)
            finishedTxs.add(rec.txNumber());
         // If a transaction has not finished, undo it and also remove it from the non-quiescent checkpoint list.
         else if (!finishedTxs.contains(rec.txNumber())) {
            rec.undo(rec.txNumber());
            if (rec.txNumber() != -1 && flag)
               nQCheckpointsTransactions.remove(nQCheckpointsTransactions.indexOf(rec.txNumber()));
         }
         finishedTxs.add(rec.txNumber());
      }
   }
